...
async function loadProductsForAdmin() {
  ...
  try {
    ...
    document.querySelectorAll(".delete-btn").forEach((b) =>
      b.addEventListener("click", deleteProduct)
    );
    document.querySelectorAll(".edit-product-btn").forEach((b) =>
      b.addEventListener("click", setupEditModal)
    );
  } catch (error) {
    console.error("Error loading products:", error);
    adminProductTable.innerHTML =
      '<tr><td colspan="5" class="text-center text-danger">Error connecting to product API.</td></tr>';
  }
}
...
async function loadUsers() {
  ...
  try {
    ...
    try {
      ...
      console.warn("Invalid token format, skipping decode:", err);
    }
